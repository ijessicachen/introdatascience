...
class Domain():
    ...
    * A domain built from json file (where is its description)
    ...
    def people_desired_velocity(self, xyr, people_dest, II=None, JJ=None):
        ...
        """
        # check DVX Shape format
        #print("DVX SHAPE", self.destinations['a1'].desired_velocity_X.shape)
        ...
        # term with local density 
        ld = np.zeros((xyr.shape[0], 2))
        # V0
        V0 = np.zeros((xyr.shape[0], 2))

        for dest_name in np.unique(people_dest):

            # hopefully fix people getting stuck in empty corridors
            Ny, Nx = self.destinations[dest_name].desired_velocity_X.shape

            II = np.clip(II, 0, Ny - 1)
            JJ = np.clip(JJ, 0, Nx - 1)
            ...
            xdir = self.destinations[dest_name].desired_velocity_X[II[ind], JJ[ind]]
            ydir = self.destinations[dest_name].desired_velocity_Y[II[ind], JJ[ind]]

            # Vd - original desired velocity
            Vd[ind, 0] = xyr[ind, 3]*scale*xdir
            Vd[ind, 1] = xyr[ind, 3]*scale*ydir

            # V0
            # magnitude must be 1.34
            V0[ind, 0] = 1.34*scale*xdir
            V0[ind, 1] = 1.34*scale*ydir

            # desired velocities
            print("Vd", Vd[ind, 0], Vd[ind, 1])
            print("V0", V0[ind, 0], V0[ind, 1])
            ...
            sigma = 0.6
            for i in range(0, ind.shape[0]):
                ld[ind[i], 0] = np.sum( (1/(2*np.pi*sigma**2)) *\
                                np.exp(-np.multiply(dx[i], dx[i])/(2*sigma**2)))
                ld[ind[i], 1] = np.sum( (1/(2*np.pi*sigma**2)) *\
                                np.exp(-np.multiply(dy[i], dy[i])/(2*sigma**2)))
            ...
            for i in range(0, ind.shape[0]):
                if (dd[i][0]):
                    Vd[ind[i], 0] = np.multiply(V0[ind[i], 0], ld[ind[i], 0])
                if (dd[i][1]):
                    Vd[ind[i], 1] = np.multiply(V0[ind[i], 0], ld[ind[i], 1])

            vd_norm = np.linalg.norm(Vd, axis=1)
            if np.any(vd_norm < 1e-3):
                print("Near-zero Vd for indices:", np.where(vd_norm < 1e-3))
